...
if($from == date('Y-m-d', strtotime('15 day')))
if(isset($_GET['range'])){
  $range = $_GET['range'];
  $ex = explode('-', $range);
                    ...
                    <div class="container-fluid">
                        ...
                        <div class="page-title-box">
                            <div class="row align-items-center">
                                <div class="col-md-8">
                                    <h6 class="page-title"><i class="ti ti-credit-card"></i> Payroll Records</h6>
                                  <ol class="breadcrumb m-0">
                                        <li class="breadcrumb-item"><a href="#">List of Payroll <i class="mdi mdi-arrow-down"></i></a></li>
                  ...
                  ?>
                                        <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                            ...
                                            <tbody>
                                                ...
                                                $i = 1;
                                                if($role == '1') {
                                                $attendance = mysqli_query($conn,"SELECT *, sum(number_hr_morning) AS morning, sum(number_hr_afternoon) AS afternoon, tbl_attendance_log.user_id AS empid FROM tbl_attendance_log LEFT JOIN tbl_user  ON tbl_user.id=tbl_attendance_log.user_id LEFT JOIN tbl_work_position ON tbl_work_position.id=tbl_user.position_id WHERE date_in BETWEEN '$from' AND '$to' GROUP BY tbl_attendance_log.user_id ORDER BY tbl_user.fname ASC") or die (mysqli_error($conn));
                                            }elseif($role == '2') {
                                                $attendance = mysqli_query($conn,"SELECT *, sum(number_hr_morning) AS morning, sum(number_hr_afternoon) AS afternoon, tbl_attendance_log.user_id AS empid FROM tbl_attendance_log LEFT JOIN tbl_user  ON tbl_user.id=tbl_attendance_log.user_id LEFT JOIN tbl_work_position ON tbl_work_position.id=tbl_user.position_id WHERE user_id = '$user_id' AND date_in BETWEEN '$from' AND '$to' GROUP BY tbl_attendance_log.user_id ORDER BY tbl_user.fname ASC") or die (mysqli_error($conn));
                                            }
                                                while($attendanceRow = mysqli_fetch_assoc($attendance)) {
                                                $id = $attendanceRow['empid'];           
                                                $user_id = $attendanceRow['user_id'];
                                                $total_hr = $attendanceRow['morning'] + $attendanceRow['afternoon'];
